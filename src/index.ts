...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    h1 { font-size: 2.5rem; font-weight: 500; letter-spacing: -0.02em; margin-bottom: 0.5rem; }
    .subtitle { font-size: 1.1rem; color: var(--text-secondary); font-style: italic; }
    .agents { display: flex; flex-direction: column; gap: 1px; background: var(--border); border: 1px solid var(--border); margin-bottom: 3rem; }
    .agent { background: var(--surface); padding: 1.5rem 2rem; display: grid; grid-template-columns: 1fr auto; gap: 2rem; align-items: center; }
    .agent:hover { background: var(--bg); }
    .agent-info { display: flex; flex-direction: column; gap: 0.25rem; }
    .agent-name { font-size: 1.1rem; font-weight: 500; }
    .agent-desc { font-size: 0.95rem; color: var(--text-secondary); }
    .agent-meta { display: flex; align-items: center; gap: 0.5rem; margin-top: 0.5rem; }
    .agent-schedule { font-family: 'IBM Plex Mono', monospace; font-size: 0.75rem; color: var(--text-tertiary); background: var(--bg); padding: 0.25rem 0.5rem; }
    .agent-status { font-family: 'IBM Plex Mono', monospace; font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.05em; padding: 0.25rem 0.5rem; }
    ...
    .btn { font-family: 'IBM Plex Mono', monospace; font-size: 0.8rem; padding: 0.6rem 1.2rem; border: 1px solid var(--border); background: var(--surface); color: var(--text); cursor: pointer; }
    .btn:hover { border-color: var(--text); background: var(--text); color: var(--surface); }
    .btn:disabled { opacity: 0.4; cursor: not-allowed; }
    .output-section { border: 1px solid var(--border); background: var(--surface); }
    .output-header { display: flex; justify-content: space-between; align-items: center; padding: 1rem 1.5rem; border-bottom: 1px solid var(--border); }
    .output-title { font-family: 'IBM Plex Mono', monospace; font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.1em; color: var(--text-tertiary); }
    .output-content { font-family: 'IBM Plex Mono', monospace; font-size: 0.8rem; line-height: 1.8; padding: 1.5rem; min-height: 200px; max-height: 400px; overflow-y: auto; color: var(--text-secondary); }
    ...
    .empty-state { color: var(--text-tertiary); font-style: italic; font-family: 'Newsreader', Georgia, serif; }
    .toast { position: fixed; bottom: 2rem; right: 2rem; font-family: 'IBM Plex Mono', monospace; font-size: 0.8rem; padding: 1rem 1.5rem; background: var(--text); color: var(--surface); opacity: 0; transform: translateY(8px); transition: all 0.2s ease; }
    .toast.show { opacity: 1; transform: translateY(0); }
    .toast.error { background: var(--error); }
    @media (max-width: 768px) { .container { padding: 2rem 1rem; } .agent { grid-template-columns: 1fr; gap: 1rem; } }
  </style>
</head>
<body>
  <div class="container">
...
function appendOutput(text, type) {
  ...
  if (o.querySelector(".empty-state")) o.innerHTML = "";
...
function runAgent(key, retry) {
  retry = retry || 0;
  agentStatus[key] = "running";
  renderAgents();
  if (retry === 0) {
    appendOutput("Warming up container...", "info");
    fetch("/api/agents/" + key + "/warm", { method: "POST" }).then(function() {
...
function doRun(key, retry) {
  fetch("/api/agents/" + key + "/run", { method: "POST" }).then(function(r) { return r.json(); }).then(function(d) {
    if (d.success) {
      ...
      appendOutput("Container starting, retry in 10s (attempt " + (retry + 2) + "/4)", "info");
      setTimeout(function() { doRun(key, retry + 1); }, 10000);
      return;
    } else {
      appendOutput("Failed: " + d.error, "error");
    ...
    if (retry < 3) {
      appendOutput("Retrying in 10s (attempt " + (retry + 2) + "/4)", "info");
      setTimeout(function() { doRun(key, retry + 1); }, 10000);
      return;
    }
...
function viewLogs(key) {
  ...
  }).catch(function(e) { appendOutput("Error: " + e.message, "error"); });
}
